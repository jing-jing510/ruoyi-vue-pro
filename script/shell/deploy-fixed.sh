...
function backup() {
    ...
    if [ ! -d "$BASE_PATH/backup" ]; then
        echo "[backup] 创建备份目录 $BASE_PATH/backup"
...
function transfer() {
    ...
    if [ ! -d "$SOURCE_PATH" ]; then
        echo "[transfer] 错误：$SOURCE_PATH 目录不存在"
        exit 1
    fi
    
    # 检查源 jar 包是否存在
    if [ ! -f "$SOURCE_PATH/$SERVER_NAME.jar" ]; then
        echo "[transfer] 错误：$SOURCE_PATH/$SERVER_NAME.jar 不存在"
        echo "[transfer] 请先将编译好的 jar 包放到 $SOURCE_PATH 目录"
        exit 1
    fi
...
function deploy() {
    # 确保基础目录存在
    if [ ! -d "$BASE_PATH" ]; then
        echo "[deploy] 创建基础目录 $BASE_PATH"
        mkdir -p $BASE_PATH
    fi
    
    # 确保 heapError 目录存在
    if [ ! -d "$HEAP_ERROR_PATH" ]; then
        echo "[deploy] 创建 heapError 目录 $HEAP_ERROR_PATH"
