...
/** 红林生产日报信息 */
export interface HLProductionDailyReport {
          id: number; // 日报id
          ...
          remarks: string; // 备注信息
          reservedField1: string; // 调度值长
          reservedField2: string; // 副值长
  }

// 红林生产日报 API
export const HLProductionDailyReportApi = {
  // 查询红林生产日报分页
  getHLProductionDailyReportPage: async (params: any) => {
    return await request.get({ url: `/coal/HL-production-daily-report/page`, params })
  },

  // 查询红林生产日报详情
  getHLProductionDailyReport: async (id: number) => {
    return await request.get({ url: `/coal/HL-production-daily-report/get?id=` + id })
  },

  // 新增红林生产日报
  createHLProductionDailyReport: async (data: HLProductionDailyReport) => {
    return await request.post({ url: `/coal/HL-production-daily-report/create`, data })
  },

  // 修改红林生产日报
  updateHLProductionDailyReport: async (data: HLProductionDailyReport) => {
    return await request.put({ url: `/coal/HL-production-daily-report/update`, data })
  },

  // 删除红林生产日报
  deleteHLProductionDailyReport: async (id: number) => {
    return await request.delete({ url: `/coal/HL-production-daily-report/delete?id=` + id })
  },

  /** 批量删除红林生产日报 */
  deleteHLProductionDailyReportList: async (ids: number[]) => {
    return await request.delete({ url: `/coal/HL-production-daily-report/delete-list?ids=${ids.join(',')}` })
  },

  // 导出红林生产日报 Excel
  exportHLProductionDailyReport: async (params) => {
    return await request.download({ url: `/coal/HL-production-daily-report/export-excel`, params })
  },
}