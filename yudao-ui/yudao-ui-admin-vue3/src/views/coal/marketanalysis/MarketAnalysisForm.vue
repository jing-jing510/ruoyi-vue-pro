<template>
  <Dialog :title="dialogTitle" v-model="dialogVisible">
    ...
    >
      ...
      <el-form-item label="市场分析文件" prop="reportFileUrl">
        <UploadFile
          ...
          :autoUpload="true"
          :drag="false"
          directory="coal_market_analysis_file"
        />
      </el-form-item>
      <el-form-item label="图片上传" prop="imageUrls">
        <UploadImgs
          ...
          :limit="9"
          :fileSize="10"
          directory="coal_market_analysis_image"
        />
      </el-form-item>
...
import { MarketAnalysisApi, MarketAnalysis } from '@/api/coal/marketanalysis'
import UploadFile from '@/components/UploadFile/src/UploadFile.vue'
import UploadImgs from '@/components/UploadFile/src/UploadImgs.vue'

/** 市场分析 表单 */
defineOptions({ name: 'MarketAnalysisForm' })
...
const formRules = reactive({
  ...
  reportFileUrl: [{ required: true, message: '请上传市场分析文件', trigger: 'change' }],
})
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await MarketAnalysisApi.getMarketAnalysis(id) as unknown as FormVO
    } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as MarketAnalysis
    if (formType.value === 'create') {
      await MarketAnalysisApi.createMarketAnalysis(data)
      message.success(t('common.createSuccess'))
    } else {
      await MarketAnalysisApi.updateMarketAnalysis(data)
