<template>
  <Dialog :title="dialogTitle" v-model="dialogVisible">
    ...
    >
      ...
      <el-form-item label="洗选报告文件" prop="reportFileUrl">
        <UploadFile
          ...
          :autoUpload="true"
          :drag="false"
          directory="coal_market_washing_file"
        />
      </el-form-item>
      <el-form-item label="图片上传" prop="imageUrls">
        <UploadImgs
          ...
          :limit="9"
          :fileSize="10"
          directory="coal_market_washing_image"
        />
      </el-form-item>
...
/** 洗选分析报告 表单 */
defineOptions({ name: 'MarketWashingReportForm' })
...
const formRules = reactive({
  ...
  reportFileUrl: [{ required: true, message: '请上传洗选报告文件', trigger: 'change' }],
})
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await MarketWashingReportApi.getMarketWashingReport(id) as unknown as FormVO
    } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as MarketWashingReport
    if (formType.value === 'create') {
      await MarketWashingReportApi.createMarketWashingReport(data)
      message.success(t('common.createSuccess'))
    } else {
      await MarketWashingReportApi.updateMarketWashingReport(data)
