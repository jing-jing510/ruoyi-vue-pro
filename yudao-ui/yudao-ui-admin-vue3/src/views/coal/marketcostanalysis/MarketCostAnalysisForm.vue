<template>
  <Dialog :title="dialogTitle" v-model="dialogVisible">
    ...
    >
      ...
      <el-form-item label="成本核算文件" prop="reportFileUrl">
        <UploadFile
          ...
          :autoUpload="true"
          :drag="false"
          directory="coal_market_cost_file"
        />
      </el-form-item>
      <el-form-item label="图片上传" prop="imageUrls">
        <UploadImgs
          ...
          :limit="9"
          :fileSize="10"
          directory="coal_market_cost_image"
        />
      </el-form-item>
...
/** 综合成本核算 表单 */
defineOptions({ name: 'MarketCostAnalysisForm' })
...
const formRules = reactive({
  ...
  reportFileUrl: [{ required: true, message: '请上传成本核算文件', trigger: 'change' }],
})
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await MarketCostAnalysisApi.getMarketCostAnalysis(id) as unknown as FormVO
    } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as MarketCostAnalysis
    if (formType.value === 'create') {
      await MarketCostAnalysisApi.createMarketCostAnalysis(data)
      message.success(t('common.createSuccess'))
    } else {
      await MarketCostAnalysisApi.updateMarketCostAnalysis(data)
