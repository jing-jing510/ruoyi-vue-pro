<template>
    ...
    >
      ...
      <el-form-item>
        ...
        <el-button
          ...
          v-hasPermi="['coal:HL-production-daily-report:create']"
        >
        ...
        <el-button
          ...
          v-hasPermi="['coal:HL-production-daily-report:export']"
        >
        ...
        <el-button
            ...
            v-hasPermi="['coal:HL-production-daily-report:delete']"
        >
  ...
  <ContentWrap>
    ...
    <el-table-column type="selection" width="55" />
      <el-table-column
        ...
        width="120px"
      />
      <el-table-column label="集控员" align="center" prop="operatorId" width="100px">
        <template #default="scope">
          {{ getUserName(scope.row.operatorId) }}
        </template>
      </el-table-column>
      <el-table-column label="带班主任" align="center" prop="shiftLeaderId" width="100px">
        <template #default="scope">
          {{ getUserName(scope.row.shiftLeaderId) }}
        </template>
      </el-table-column>
      <el-table-column
        label="启车时间"
      ...
      <el-table-column label="调度值长" align="center" prop="reservedField1" />
      <el-table-column label="副值长" align="center" prop="reservedField2" />
      <el-table-column
        label="创建时间"
        align="center"
        prop="createTime"
      ...
      <el-table-column label="操作" align="center" min-width="120px">
        <template #default="scope">
          <el-button
            ...
            v-hasPermi="['coal:HL-production-daily-report:update']"
          >
            编辑
          </el-button>
          <el-button
  ...
  <!-- 表单弹窗：添加/修改 -->
  <HLProductionDailyReportForm ref="formRef" @success="getList" />
</template>
...
import HLProductionDailyReportForm from './HLProductionDailyReportForm.vue'
import { getSimpleUserList, UserVO } from '@/api/system/user'

/** 红林生产日报 列表 */
defineOptions({ name: 'HLProductionDailyReport' })

const message = useMessage() // 消息弹窗
const { t } = useI18n() // 国际化

const loading = ref(true) // 列表的加载中
const list = ref<HLProductionDailyReport[]>([]) // 列表的数据
const total = ref(0) // 列表的总页数
const userList = ref<UserVO[]>([]) // 用户列表
const userMap = ref<Map<number, string>>(new Map()) // 用户ID到名称的映射
...
const getList = async () => {
  loading.value = true
  try {
    const data = await HLProductionDailyReportApi.getHLProductionDailyReportPage(queryParams)
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await HLProductionDailyReportApi.deleteHLProductionDailyReport(id)
...
const handleDeleteBatch = async () => {
  try {
    // 删除的二次确认
    await message.delConfirm()
    await HLProductionDailyReportApi.deleteHLProductionDailyReportList(checkedIds.value);
    checkedIds.value = [];
    message.success(t('common.delSuccess'))
    await getList();
  } catch {}
...
const handleRowCheckboxChange = (records: HLProductionDailyReport[]) => {
...
const handleExport = async () => {
  try {
    ...
    const data = await HLProductionDailyReportApi.exportHLProductionDailyReport(queryParams)
    download.excel(data, '红林生产日报.xls')
  } catch {
  } finally {
...
const openOnlineReport = () => {
  ...
  const url = `${baseUrl}/jmreport/shareView/1120595319747600384?shareToken=75f9380791342fcfcaadfae8e3f8b701`
...
const loadUserList = async () => {
  try {
    ...
    userList.value = data
    // 构建用户ID到名称的映射
    userMap.value = new Map(data.map(user => [user.id, user.nickname]))
  } catch (error) {
...
const getUserName = (userId: number) => {
  return userMap.value.get(userId) || userId
}
